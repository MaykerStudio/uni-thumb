...
into a distributable tarball at dist/com.maykerstudio.unithumb-<version>.tgz.
...
#   entries, zero "Samples~" entries.  Repo-only content (CONTRIBUTING.md,
#   ROADMAP.md, Documentation~/images/) is excluded from the tarball.
# - The package.json "files" allowlist is the Git-URL install channel mechanism;
#   this script governs the tarball distribution channel.
...
# dev tooling - none ship in the tarball. Documentation~/ holds doc images
# (never shipped); images/ stays guarded at any depth as well.
$ExcludedDirNames = @(
    '.git', '.github', '.vs', '.vscode', '.idea', 'Temp',
...
$ExcludedFileNames = @(
    ...
    'AGENTS.md', 'CONTRIBUTING.md', 'ROADMAP.md'
)
...
function Test-TarballValid {
    ...
    - zero root-level CONTRIBUTING.md and ROADMAP.md entries
    - zero images/ (any depth) and Documentation~/ entries
    ...
    foreach ($entry in $Entries) {
        ...
        if ($segments.Count -eq 2 -and $segments[1] -eq 'CONTRIBUTING.md') {
            $contributingCount++
        }
        if ($segments.Count -eq 2 -and $segments[1] -eq 'ROADMAP.md') {
            $roadmapCount++
        }
        if ($segments -contains 'images') {
            $imagesCount++
        }
        if ($segments -contains 'Documentation~') {
            $documentationCount++
        }
    }
    ...
    if ($contributingCount -gt 0) {
        $reasons.Add("$contributingCount CONTRIBUTING.md entries found")
    }
    if ($roadmapCount -gt 0) {
        $reasons.Add("$roadmapCount ROADMAP.md entries found")
    }
    if ($imagesCount -gt 0) {
        $reasons.Add("$imagesCount images/ entries found")
    }
    if ($documentationCount -gt 0) {
        $reasons.Add("$documentationCount Documentation~/ entries found")
    }
...
function Invoke-NpmPack {
    ...
    try {
        # Under PS 5.1, native stderr lines become ErrorRecords via 2>&1.
        # With EAP=Stop these throw before $LASTEXITCODE is checked.
        # Temporarily switch to Continue so stderr notice text is harmless.
...
try {
    ...
    Write-ProgressLog "validation: package.json=$($validation.hasPackageJson) Editor=$($validation.hasEditor) Examples=$($validation.examplesCount) Project=$($validation.projectCount) docs=$($validation.docsCount) unity=$($validation.unityCount) prefab=$($validation.prefabCount) meta=$($validation.metaCount) Samples~=$($validation.samplesCount) contributing=$($validation.contributingCount) roadmap=$($validation.roadmapCount) images=$($validation.imagesCount) Documentation~=$($validation.documentationCount)"
