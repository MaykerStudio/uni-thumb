...
namespace MaykerStudio.UniThumb
{
    ...
    ///
    /// Cache invalidation: per-entry Ticks track the scene file's
    /// File.GetLastWriteTimeUtc.Ticks at cache time. Load compares the live
    /// scene file ticks against the cached record and reloads from disk when
    /// stale. Load never regenerates: this tool is strictly manual-only.
    ///
    /// Cache ownership: the cache owns every Texture2D it returns (runtime textures,
    /// HideAndDontSave). DestroyImmediate happens on eviction (Save/Delete),
    /// on size-cap LRU eviction (cap from UniThumbSettings.CacheSizeMb, least-recently-touched first) and on
    ...
    public static class UniThumbStorage
    {
        ...
        // Legacy default (256 MB) kept for documentation only. The actual cap is
        // now read from UniThumbSettings.CacheSizeMb at eviction time.
        ...
        /// TryGetCachedTexture hits. ByteSize is width * height * 4 (long to
        /// avoid overflow on large textures), valid only
        ...
        /// Library/SceneThumbnails/ or Assets/UniThumb/Thumbnails/), warms the
        ...
        public static bool Save(string scenePath, byte[] pngBytes)
        {
            ...
            long sceneTicks = TryGetSceneTicks(scenePath);
        ...
        /// staleness - no PNG read. A stale scene file
        ...
        /// <summary>
        /// Cache-only lookup for overlay callbacks: zero file I/O,
        /// zero allocations. Returns the cached instance
        ...
        /// <summary>
        /// Removes the thumbnail PNG. The cached texture is dropped first (cache
        /// removal, then DestroyImmediate); callers repaint after the call. Library
        /// mode deletes the raw file (File.Delete only, no AssetDatabase involvement).
        /// Tracked mode uses AssetDatabase.DeleteAsset so the PNG and its .meta are
        /// removed cleanly, with a raw File.Delete of png + .meta fallback when the
        /// asset path is invalid (not yet imported). Never calls AssetDatabase.Refresh.
        ...
        /// <summary>
        /// True when a thumbnail PNG exists but the source asset file was
        /// modified after the PNG was written (source LastWriteTimeUtc ticks
        /// newer than the PNG ticks). Missing PNGs are NOT stale (that is
        /// IsMissing territory). Uses the same File.GetLastWriteTimeUtc tick
        /// clock as Load/TryGetSceneTicks - no new clock. Works for scene and
        /// prefab paths alike (both resolve through the GUID PNG name).
        /// </summary>
        internal static bool IsThumbnailStale(string assetPath)
        {
            ...
            long sourceTicks = TryGetSceneTicks(assetPath);
            if (sourceTicks == 0L)
            {
                return false;
            }
            ...
            try
            {
                pngTicks = File.GetLastWriteTimeUtc(pngPath).Ticks;
            }
            catch (Exception)
            {
                return false;
            }
            return sourceTicks > pngTicks;
        }

        /// <summary>
        /// GUID-keyed prefab thumbnail API. Prefabs render without opening as a
        /// scene, so there is no scene path to key on: these entries are named
        /// {prefabGuid}.png in the same ACTIVE storage folder and share the
        /// ownership, LRU, and eviction rules of the scene entries (GUID keys
        /// survive prefab renames and moves). Ticks come from the prefab asset
        /// file via GUIDToAssetPath; an unknown GUID records 0 ticks. Callers
        /// must never destroy textures from LoadPrefabThumbnail.
        /// </summary>
        public static bool SavePrefabThumbnail(string guid, byte[] pngBytes)
        {
            if (pngBytes == null || pngBytes.Length == 0)
            {
                Debug.LogWarning(
                    k_LogPrefix + "SavePrefabThumbnail refused: PNG byte buffer is null or empty."
                );
                return false;
            }
            if (string.IsNullOrEmpty(guid))
            {
                Debug.LogWarning(
                    k_LogPrefix
                        + "SavePrefabThumbnail refused: prefab GUID is null or empty. No PNG written."
                );
                return false;
            }
            ...
            long sourceTicks = TryGetAssetTicksByGuid(guid);
            Texture2D texture = CreateTextureFromBytes(pngBytes);
            if (texture != null)
            {
                InsertIntoCache(guid, texture, sourceTicks);
            }
            return true;
        }

        /// <summary>
        /// Returns the cached Texture2D reference for a prefab GUID (owned by
        /// the cache), reloading from disk when stale or not yet cached.
        /// Returns null when no PNG exists for the GUID.
        /// </summary>
        public static Texture2D LoadPrefabThumbnail(string guid)
        {
            if (string.IsNullOrEmpty(guid))
            {
                return null;
            ...
            if (s_Cache.TryGetValue(guid, out entry) && entry.Texture != null)
            {
                if (sourceTicks == entry.Ticks)
                {
                    return entry.Texture;
                }
                Evict(guid);
            }
            return LoadFromDisk(guid, sourceTicks);
        }

        /// <summary>
        /// True when a thumbnail mapping exists for the prefab GUID (cache
        /// first, then a disk existence check).
        /// </summary>
        public static bool HasPrefabThumbnail(string guid)
        {
            if (string.IsNullOrEmpty(guid))
            {
                return false;
        ...
        /// <summary>
        /// Removes the thumbnail PNG for a prefab GUID, dropping the cached
        /// texture first. Mirrors Delete's mode handling (AssetDatabase in
        /// tracked mode, raw File.Delete in Library mode). Returns true when
        /// the PNG file was deleted.
        /// </summary>
        public static bool DeleteByGuid(string guid)
        {
            if (string.IsNullOrEmpty(guid))
            {
                return false;
        ...
        /// <summary>
        /// Asset-file timestamp for GUID-keyed (prefab) cache records. Resolves
        /// the assets-relative path via GUIDToAssetPath, then reuses the same
        /// file-ticks rule as scenes. Unknown GUIDs yield 0 (never a crash).
        /// </summary>
        private static long TryGetAssetTicksByGuid(string guid)
        {
            ...
            return TryGetSceneTicks(assetPath);
        }

        /// <summary>
        /// Scene-file timestamp for cache staleness records. Never throws: a
        /// missing or unreadable scene file yields 0 (treated as stale on the
        /// next comparison, never as a crash).
        /// </summary>
        private static long TryGetSceneTicks(string scenePath)
        {
            try
            {
                string absolute = AbsoluteProjectPath(scenePath);
                if (!File.Exists(absolute))
                {
                    return 0L;
                }
                return File.GetLastWriteTimeUtc(absolute).Ticks;
            }
            catch (Exception)
            {
                return 0L;
        ...
        private static void InsertIntoCache(string guid, Texture2D texture, long sceneTicks)
        {
            var entry = new CacheEntry
            {
                ...
                ByteSize = (long)texture.width * texture.height * 4,
            };
            s_Cache[guid] = entry;
            s_State.TotalBytes += entry.ByteSize;
        ...
        {
            long cap = (long)UniThumbSettings.Get().CacheSizeMb << 20;
