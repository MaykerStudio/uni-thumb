...
namespace MaykerStudio.UniThumb
{
    ...
    public static class ExampleSceneGenerator
    {
        ...
        internal enum RendererDimension
        {
            D2D,
            D3D,
        }
        ...
        // Static renderer-dimension probe result for this generation run only;
        // reset at the start of GenerateExampleScenes. Static (not a guard
        // holder) because the helpers are all static menu entry points.
        ...
        private static void EnsureScenesFolder()
        {
            ...
            // Create the full parent chain (Assets/UniThumb, then Examples)
            // so generation works when neither folder exists yet.
            string[] parts = k_ScenesFolder.Split('/');
        ...
        internal static RendererDimension DetectRendererDimension()
        {
#if HAS_URP
            try
            {
                // Shim path: typed pipeline/renderer reads live in UrpBridge;
                // -1 means no Universal pipeline asset is active (3D path).
                int dimension = UniThumbUrp.DetectRendererDimension();
                if (dimension < 0)
                {
                    return RendererDimension.D3D;
                }
                string pipelineName = UniThumbUrp.ActivePipelineName();
                string rendererName = UniThumbUrp.ActiveRendererName();
                RendererDimension result =
                    dimension == 1 ? RendererDimension.D2D : RendererDimension.D3D;
                Debug.Log(
                    "[UniThumb] Render pipeline '"
                        + pipelineName
                        + "' resolved to "
                        + (result == RendererDimension.D2D ? "2D" : "3D")
                        + " renderer ("
                        + rendererName
                        + ")."
                );
            ...
            {
                Debug.LogWarning(
                    "[UniThumb] Renderer dimension detection failed ("
                        + ex.GetType().Name
                        + ": "
                        + ex.Message
                        + "); defaulting to 3D."
...
#else
            // Built-in RP and HDRP share the 3D path (the 2D renderer is URP-only).
            return RendererDimension.D3D;
#endif
        }
...
#if HAS_URP
            // Shim path: typed Light2D setup lives in UrpBridge (the old
            // Experimental namespace keeps deserializing via MovedFrom, so
            // existing example scenes stay compatible).
            UniThumbUrp.SetupExampleLight2D(go, color, intensity, outerRadius);
#else
            // Built-in RP and HDRP have no Light2D; use a standard point light.
            Debug.LogWarning(
