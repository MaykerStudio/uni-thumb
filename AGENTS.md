...
Project/ - Unity dev project (gitignored, never packaged): Assets/, ProjectSettings/, Packages/
```

- The repository root IS the UPM package root: `package.json` at root, plus `CHANGELOG.md`, `README.md`, `LICENSE.md`, `images/`.
- `Project/Assets/UniThumb/` is runtime-created in the dev project, not part of the package: it holds the tool's settings asset (`UniThumbSettings.asset`, auto-created) and generated example scenes. `Project/Packages/manifest.json` pins `com.maykerstudio.unithumb` to `file:../..` (the repo root).
- The package ships no example scenes. Example scenes are created on demand: `Tools > UniThumb > Generate Example Scenes` writes 10 scenes to `Assets/UniThumb/Examples/` in the consuming Unity project (here: `Project/`).
